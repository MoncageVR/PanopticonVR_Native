...
AJailBuilding::AJailBuilding()
{
	...
	if (Jail1FWeaponDoor && Jail2FWeaponDoor && Jail3FWeaponDoor)
	{
		...
		Jail1FWeaponDoor->SetRelativeLocation(FVector(0.0f, 0.0f, 667.0f));
		...
		Jail2FWeaponDoor->SetRelativeLocation(FVector(0.0f, 0.0f, 1597.0f));
		...
		Jail3FWeaponDoor->SetRelativeLocation(FVector(0.0f, 0.0f, 2527.0f));
...
void AJailBuilding::BeginPlay()
{
	...
	{
		CLSubdueHatch->OnComponentBeginOverlap.AddDynamic(this, &AJailBuilding::OverlapHatchBoxBegin);
...
void AJailBuilding::HandleJailReceiveByABButton(FName InTargetName)
{
	...
	{
		//UE_LOG(LogTemp, Log, TEXT("TopEscape State Prisoner Subdue Logic Play Parts!"));
