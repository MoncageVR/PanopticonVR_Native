#include "EquipmentActor/CarTypePanel_Actors/ACarPanel.h"
...
AACarPanel::AACarPanel()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_MainHandle(TEXT("/Game/VRContent/Modeling/03_CarTypePanel/SM_Car_MainHandle.SM_Car_MainHandle"));
	if (ModelingFinder_MainHandle.Succeeded())
	{
		...
		ActorBaseMesh->SetRelativeLocation(FVector(0.0f, 15.0f, 20.5f));
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_MainBody(TEXT("/Game/VRContent/Modeling/03_CarTypePanel/SM_Car_Main_Body.SM_Car_Main_Body"));
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_SubHandle(TEXT("/Game/VRContent/Modeling/03_CarTypePanel/SM_Car_SubHandle.SM_Car_SubHandle"));
	...
	if (CarKeyGC)
	{
		CarKeyGC->SetupAttachment(CarMainBody);
		CarKeyGC->SetRelativeLocation(FVector(15.0f, 1.7f, -1.2f));
		CarKeyGC->SetRelativeRotation(FRotator(90.0f, 0.f, 0.f));
		CarKeyGC->EEGrabType = EGrabType::SpawnObj;
	}

	// Setting KeyHole And Body
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_KeyHole(TEXT("/Game/VRContent/Modeling/03_CarTypePanel/SM_Car_KeyHole.SM_Car_KeyHole"));
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_KeyHoleBody(TEXT("/Game/VRContent/Modeling/03_CarTypePanel/SM_Car_KeyHoleBody.SM_Car_KeyHoleBody"));
	...
	// Setting Glasses
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_GlassOne(TEXT("/Game/VRContent/Modeling/03_CarTypePanel/SM_Car_Glass1.SM_Car_Glass1"));
	...
	if (CarPanelGlass1)//&& CarPanelGlass2)
	{
		...
		if (ModelingFinder_GlassOne.Succeeded())
		{
...
void AACarPanel::BeginPlay()
{
	...
	for (TScriptInterface<IIEquipmentInitInterface> Equip : EquipmentWorldSubSystem->GetEquipmentArr())
	{
		...
		LinkedCarCrane = Cast<AACarCrane>(IEquipPtr);
		if (LinkedCarCrane)
			break;
		else
			continue;
	...
	MapObjManagerRef = GetWorld()->GetGameInstance()->GetSubsystem<UMapObjManagerSubsystem>();
	checkf(MapObjManagerRef, TEXT("In CarPanel, MapObjManager is Not Valid!"));
}
...
void AACarPanel::Tick(float DeltaTime)
{
	...
	if (bIsHandingMainHandle)
	{
		if (LinkedCarCrane && ActorBaseMesh)
		{
			float CurrentYaw = ActorBaseMesh->GetRelativeRotation().Yaw;
			float DeltaYaw = FMath::FindDeltaAngleDegrees(PrevHandleYaw, CurrentYaw);
			PrevHandleYaw = CurrentYaw;

			LinkedCarCrane->HandleAddCraneYaw(DeltaYaw * 0.25f);
		}
	}
...
void AACarPanel::OnGrabbed(UMotionControllerComponent& InMCRef, const FVector& HandGrabPos, class AVRHand* InGrabbingHand)
{
	if (GC->MCRef == &InMCRef)
	{
		...
		{
			bIsHandingMainHandle = true;

			if (InMCRef.ComponentHasTag(FName("RightMC")))
			{
				FVector MHGCRightGrabbedPos = AtFirstMainHandleGCVec + FVector(16.5f, 4.0f, -14.0f);
...
void AACarPanel::OnDropped()
{
	...
	{
		GetWorld()->GetTimerManager().PauseTimer(EraseGolfEffectBySubHandleOperateTimer);
		GetWorld()->GetTimerManager().ClearTimer(EraseGolfEffectBySubHandleOperateTimer);
		...
		SubHandleGC->MCRef = nullptr;
	}
	else if (CarKeyGC->MCRef)
	{
		CarKeyGC->MCRef = nullptr;
	}

	if (NewSpawnCarKey != nullptr)
	{
		NewSpawnCarKey->DetachFromActor(FDetachmentTransformRules::KeepWorldTransform);
		NewSpawnCarKey->GC->SetPrimitiveCompPhysics(true);
		NewSpawnCarKey->OnDropped();
...
void AACarPanel::SubHandleOperating()
{
	...
	if (CarSubHandle->GetRelativeRotation().Pitch <= -50.0f && !GetWorld()->GetTimerManager().IsTimerActive(EraseGolfEffectBySubHandleOperateTimer))
	{
		...
		GetWorld()->GetTimerManager().SetTimer(
			...
			this,
			&AACarPanel::CallByTimerFuncEraseGolfEffect,
			2.0f,
			true
		);
	}
		...
		if (OtherComp->ComponentHasTag(FName("CarKey")))
		{
			...
			if (bIsTurnOnCar)
			{
				if (AttachedCarKey)
				{
					...
					bIsCoolDown = 1;
					LinkedCarCrane->HandleOperatingCraneHead();
				}
			}
			else
			{
