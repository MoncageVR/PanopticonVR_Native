...
AAFuelTank::AAFuelTank()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_CellCaseBelow(TEXT("/Game/VRContent/Modeling/05_Cell(FuelRod)/SM_CellCaseBelow.SM_CellCaseBelow"));
	if (ActorBaseMesh && ModelingFinder_CellCaseBelow.Succeeded())
	{
		ActorBaseMesh->SetupAttachment(FTMainRoot);
		ActorBaseMesh->SetStaticMesh(ModelingFinder_CellCaseBelow.Object);
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ModelingFinder_CellCase(TEXT("/Game/VRContent/Modeling/05_Cell(FuelRod)/SM_CellCase.SM_CellCase"));
	FTMainBody = CreateDefaultSubobject<UStaticMeshComponent>("FuelTankMainBody");
	if (FTMainBody && ModelingFinder_CellCase.Succeeded())
	{
		FTMainBody->SetupAttachment(FTMeshRoot);
		FTMainBody->SetStaticMesh(ModelingFinder_CellCase.Object);
